...
void CDownloadItem::UpdateButtonStatus()
{
	...
	DebugPrintf(DLC_XU_KAI,"CDownloadItem::UpdateProgress m_OldState : %d, m_pItemModel->GetDownloadStatus(): %d",m_OldState,curState);
...
void CDownloadItem::InitItem()
{
	...
	m_textTaskDownload.SetText(StringManager::GetStringById(TOUCH_DOWNLOAD));
...
//void CDownloadItem::MoveWindow(INT32 _iLeft, INT32 _iTop, INT32 _iWidth, INT32 _iHeight)
//{
//	UIContainer::MoveWindow(_iLeft, _iTop, _iWidth, _iHeight);
//
//	int Width = m_textBookNameLabel.GetTextWidth();
//	int Height = m_textBookNameLabel.GetTextHeight();
//	int Top = m_VerticalMargin;
//	int textBookNameLabelWidth = 420;
//	int progressBarWidth = 420;
//	int ButtonWidth = 44;
//	int ButtonHeight = 42;
//	int ItemWidth = 600;
//	int Left = m_HorizontalMargin;
//
//	m_textBookNameLabel.MoveWindow(Left, Top, textBookNameLabelWidth, Height);
//
//	Top += (Height + 10);
//	m_proDownloadSeparatorbar.MoveWindow(Left, Top, progressBarWidth, 15);
//
//	Top += (3 + 20);
//	Width = m_textDownLoadStatus.GetTextWidth();
//	Height = m_textDownLoadStatus.GetTextHeight();
//	m_textDownLoadStatus.MoveWindow(m_HorizontalMargin, Top, 300, Height);
//
//	m_textDownLoadProgress.SetFontSize(GetWindowFontSize(UIDownloadListItemIndex));
//	m_textDownLoadProgress.SetAlign(ALIGN_RIGHT);
//	m_textDownLoadProgress.SetBackColor(ColorManager::knWhite);
//	m_textDownLoadProgress.SetBackTransparent(false);
//	char _sztmp[8] = {0};
//	m_OldPrecent = m_pItemModel->GetDownloadPercentage();
//	sprintf(_sztmp,"%d%%",m_OldPrecent);
//	m_textDownLoadProgress.SetText(_sztmp);
//	Width = m_textDownLoadProgress.GetTextWidth();
//	Height = m_textDownLoadProgress.GetTextHeight();
//	Left = m_HorizontalMargin + progressBarWidth - Width;
//	m_textDownLoadProgress.MoveWindow(Left-25, Top, Width+25, Height);
//
//	Top = 34;
//	Left = ItemWidth - m_HorizontalMargin - ButtonWidth;
//	m_btnTaskDelete.MoveWindow(Left, Top, ButtonWidth, ButtonHeight);
//	Left -= (14 + ButtonWidth);
//	m_btnTaskPause.MoveWindow(Left, Top, ButtonWidth, ButtonHeight);
//	m_btnTaskDownload.MoveWindow(Left, Top, ButtonWidth, ButtonHeight);
//
//	Top += (ButtonHeight + 10);
//	Width  = m_textTaskDelete.GetTextWidth();
//	Height = m_textTaskDelete.GetTextHeight();
//	Left += ((ButtonWidth - Width)>>1);
//	m_textTaskPause.MoveWindow(Left, Top, Width, Height);
//	m_textTaskDownload.MoveWindow(Left, Top, Width, Height);
//	Left += (14 + ButtonWidth);
//	m_textTaskDelete.MoveWindow(Left, Top, Width, Height);
//}
...
string CDownloadItem::GetDownloadStatus()
{
	...
	case IDownloadTask::WORKING:
		_szstatus = StringManager::GetStringById(DOWNLOADING);
		break;
	...
	case IDownloadTask::DONE:
		_szstatus = StringManager::GetStringById(DOWNLOAD_FINISH);
		break;
	case IDownloadTask::FAILED:
		_szstatus = StringManager::GetStringById(DOWNLOAD_FAILED);
		break;
	default:
		//_szstatus = StringManager::GetStringById(WAITING);
		break;
...
bool CDownloadList::OnItemClick(INT32 iSelectedItem)
{
	...
	//this->GetParent()->OnKeyPress(KEY_OKAY);
	return FALSE;
}
