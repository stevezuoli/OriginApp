...
Window          VKWindow;
WINDOW_COLOR    VKWindowColor = { 0, 0, {0, 220 << 8, 220 << 8, 220 << 8}
};
MESSAGE_COLOR   VKWindowAlphaColor = { 0, {0, 80 << 8, 0, 0}
};
MESSAGE_COLOR   VKWindowFontColor = { 0, {0, 0, 0, 0}
};
//XImage         *pVKLogo = NULL;
...
void DrawVKWindow (void)
{
    ...
    attrib.valuemask = 0;
    if (!pVKLogo) {
//szj xpm.h    rv = XpmCreateImageFromData (dpy, vk_xpm, &pVKLogo, &mask, &attrib);
    if (rv != XpmSuccess)
