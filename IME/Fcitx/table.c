...
//  sprintf (strPath, "f:\\data\\%s", TABLE_CONFIG_FILENAME);
    ...
    //首先来看看有多少个码表
    while (1) {
    if (!fgets (strPath, PY_PATH_MAX, fp))
        break;
...
Bool LoadTableDict (void)
{
    ...
    //首先，来看看我们该调入哪个码表
    for (i = 0; i < (unsigned int)iTableCount; i++) {
...
//  snprintf (strPath, sizeof(strPath), PKGDATADIR "/data/%s", table[iTableIMIndex].strPath);
    ...
    for (i = 0; i < (unsigned int)(table[iTableIMIndex].iCodeLength - 1); i++) {
...
//      XUnmapWindow (dpy, inputWindow);
    ...
    if (!bIsTableAddPhrase && !bIsTableDelPhrase && !bIsTableAdjustOrder) {
        ...
        if ((iCodeInputCount < table[iTableIMIndex].iCodeLength) || (table[iTableIMIndex].bHasPinyin && iCodeInputCount < table[iTableIMIndex].iPYCodeLength)) {
            ...
            if ( iCandWordCount ) {         // to realize auto-sending HZ to client
                ...
                lastFirstCandType = (CANDTYPE)tableCandWord[0].flag;  //szj
    ...
    if (iKey == ESC) {
        ...
        if ((iTableCandDisplayed == (uint)iCandWordCount && iCandWordCount < 2) || bIsInLegend)
            return IRV_DO_NOTHING;
...
char           *TableGetCandWord (int iIndex)
{
    ...
    if (str) {
        int len = strlen (str);
        if (table[iTableIMIndex].bAutoPhrase && ( len == 2 || ( len> 2 && table[iTableIMIndex].bAutoPhrasePhrase)))
        {
            //printf("TableGetCandWord is 333333333333333 str is %s\n",str);

            //UpdateHZLastInput (str);
          //  printf("TableGetCandWord is 444444444444444444\n");
        }
...
INPUT_RETURN_VALUE TableGetCandWords (SEARCH_MODE mode)
{
    ...
    if (mode == SM_FIRST) {
        ...
        if (iTableCandDisplayed == (uint)iCandWordCount)
            return IRV_DO_NOTHING;
    ...
    iCurrentCandPage = (iTableCandDisplayed == (uint)iCandWordCount) ? 0 : 1;
    iCandPageCount = (iTableCandDisplayed >= iTableTotalCandCount) ? 1 : 2;
    if ((uint)iCandWordCount == iTableTotalCandCount)
        iCandPageCount = 0;
    /* **************************************************** */
    }
    ...
    if (mode == SM_PREV) {
        ...
        if (i < 0)
        i = 0;
        for (iWidth = 0; iWidth < (uint)i; iWidth++) {
        ...
        for (iWidth = 0; iWidth < (uint)(iCandWordCount - i); iWidth++) {
        ...
        for (iWidth = i; iWidth < (uint)iCandWordCount; iWidth++) {
        if (tableCandWord[iWidth].flag == CT_NORMAL)
...
void TableAdjustOrderByIndex (int iIndex)
{
    ...
    for (iTemp = 0; iTemp < (int)strlen (table[iTableIMIndex].strInputCode); iTemp++) {
...
INPUT_RETURN_VALUE TableGetLegendCandWords (SEARCH_MODE mode)
{
    ...
    if (((mode == SM_PREV) ^ (!tableLegend->flag)) && ((iLength + 2) == (int)strlen (tableLegend->strHZ))) {
