...
//char  respath[10] = "f://data//";


//char  respath[128] = "/thunderebook/data/";
char  respath[256] =  "/mnt/us/DK_System/system/pinyin/";
...
//szj strcpy (strPath, PKGDATADIR "/data/");
...
//szj    snprintf(strPath, sizeof(strPath), "%s/.fcitx/%s",
//             getenv ("HOME"),
//             PY_USERPHRASE_FILE);
    strcpy (strPath, respath);
...
//             PY_FREQ_FILE);
    ...
    for (unsigned int i = 0; i < iPYFreqCount; i++) {
        ...
        for (unsigned int k = 0; k < pyFreqTemp->iCount; k++) {
...
INPUT_RETURN_VALUE DoPYInput (int iKey)
{
    ...
    if ((iKey >= 'a' && iKey <= 'z') || iKey == PY_SEPARATOR || (bSP && bSP_UseSemicolon && iKey == ';')) {
        ...
        if (iPYInsertPoint == (int)strlen (strFindString))
...
void CalculateCursorPosition (void)
{
    ...
    for (i = 0; i < findMap.iHZCount; i++) {
    if ((int)strlen (findMap.strPYParsed[i]) >= iTemp) {
        iCursorPos += iTemp;
        break;
    }
...
void PYResetFlags (void)
{
    ...
    freq = pyFreq->next;
    for (i = 0; i < (int)iPYFreqCount; i++) {
    hz = freq->HZList->next;
    for (j = 0; j < (int)(freq->iCount); j++) {
        hz->flag = False;
        hz = hz->next;
    }
    freq = freq->next;
...
INPUT_RETURN_VALUE PYGetCandWords (SEARCH_MODE mode)
{
    ...
    for (iVal = 0; iVal < (int)iPYFreqCount; iVal++) {
...
//      printf("系统单字%s\n", PYFAList[PYCandWords[iVal+1].cand.base.iPYFA].pyBase[PYCandWords[iVal+1].cand.base.iBase].strHZ);
...
//      printf("是系统词组%s\n", PYFAList[PYCandWords[iVal+1].cand.phrase.iPYFA].pyBase[PYCandWords[iVal+1].cand.phrase.iBase].strHZ);
        break;
        case PY_CAND_FREQ:  //是常用字
        pBase = PYCandWords[iVal].cand.freq.hz->strHZ;
//      printf("是常用字%s\n", PYCandWords[iVal+1].cand.freq.hz->strHZ);
        iType = MSG_CODE;
        break;
...
void PYCreateAuto (void)
{
    ...
    while ((int)strlen (strPYAuto) != findMap.iHZCount * 2) {
    phraseSelected = NULL;
    baseSelected = NULL;
    ...
    if ((findMap.iHZCount - iCount) > 1) {
        ...
        if (!Cmp2Map (PYFAList[candPos.iPYFA].strMap, str)) {
            ...
            for (candPos.iPhrase = 0; candPos.iPhrase < PYFAList[candPos.iPYFA].pyBase[candPos.iBase].iUserPhrase; candPos.iPhrase++) {
                ...
                return;
                if (!val || (val && ((int)strlen (phrase->strMap) == iMatchedLength))) {
                if (CheckHZCharset (phrase->strPhrase) && CheckHZCharset (PYFAList[candPos.iPYFA].pyBase[candPos.iBase].strHZ)) {
                    ...
                    else if ((int)strlen (phrase->strMap) <= (findMap.iHZCount - 1) * 2) {
        ...
        if (!Cmp2Map (PYFAList[candPos.iPYFA].strMap, str)) {
            ...
            for (candPos.iPhrase = 0; candPos.iPhrase < PYFAList[candPos.iPYFA].pyBase[candPos.iBase].iPhrase; candPos.iPhrase++) {
                ...
                if (!val && iMatchedLength == (findMap.iHZCount - 1) * 2)
                    return;
                if (!val || (val && ((int)strlen (PYFAList[candPos.iPYFA].pyBase[candPos.iBase].phrase[candPos.iPhrase].strMap) == iMatchedLength))) {
                    if (!phraseSelected) {
                    baseSelected = &(PYFAList[candPos.iPYFA].pyBase[candPos.iBase]);
                    pPYFA = &PYFAList[candPos.iPYFA];
                    phraseSelected = &(PYFAList[candPos.iPYFA].pyBase[candPos.iBase].phrase[candPos.iPhrase]);
                    }
                    else if ((int)strlen (PYFAList[candPos.iPYFA].pyBase[candPos.iBase].phrase[candPos.iPhrase].strMap) <= (findMap.iHZCount - 1) * 2) {
...
char           *PYGetCandWord (int iIndex)
{
    ...
    pIndex = (uint*)(&(PYFAList[PYCandWords[iIndex].cand.base.iPYFA].pyBase[PYCandWords[iIndex].cand.base.iBase].iIndex));
    ...
    pIndex = (uint*)(&(PYCandWords[iIndex].cand.phrase.phrase->iIndex));
    ...
    pIndex =(uint*)( &(PYCandWords[iIndex].cand.freq.hz->iIndex));
    iNewFreqCount++;
    break;
    case PY_CAND_SYMBOL:    //是特殊符号
    pBase = PYCandWords[iIndex].cand.freq.hz->strHZ;
    ...
    */
    ...
    if (iLen == findMap.iHZCount || PYCandWords[iIndex].iWhich == (unsigned int)PY_CAND_SYMBOL) {
    strPYAuto[0] = '\0';
    for (iLen = 0; iLen < (int)iPYSelected; iLen++)
...
Bool PYCheckNextCandPage (void)
{
    ...
    for (val = 0; val < (int)(pCurFreq->iCount); val++) {
        if (!hz->flag)
        return True;
        hz = hz->next;
    }
...
void PYGetSymCandWords (SEARCH_MODE mode)
{
    ...
    if (pCurFreq && pCurFreq->bIsSym) {
    hz = pCurFreq->HZList->next;
    for (i = 0; i < (int)(pCurFreq->iCount); i++) {
...
void SavePYFreq (void)
{
    ...
    //char            strPath[PY_PATH_MAX];
    char            strPathTemp[PY_PATH_MAX];
    //int             iStrLen = PY_PATH_MAX;
    FILE           *fp;
    PyFreq         *pPyFreq;
    HZ             *hz;

//szj     snprintf(strPathTemp, iStrLen, "%s/.fcitx/",
//             getenv ("HOME"));
...
//    strncat(strPathTemp, TEMP_FILE, iStrLen);
    ...
    if (!pPyFreq->bIsSym) {
        ...
        for (k = 0; k <(int)(pPyFreq->iCount); k++) {
