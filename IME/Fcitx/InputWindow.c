...
//下面的顺序不能颠倒
//szj #include "next.xpm"
//szj #include "prev.xpm"

Window          inputWindow;
int             iInputWindowX = INPUTWND_STARTX;
int             iInputWindowY = INPUTWND_STARTY;

unsigned long            iInputWindowHeight = INPUTWND_HEIGHT;
unsigned long           iFixedInputWindowWidth = 0;
unsigned long            iInputWindowWidth = INPUTWND_WIDTH;
unsigned long            iInputWindowUpWidth = INPUTWND_WIDTH;
unsigned long            iInputWindowDownWidth = INPUTWND_WIDTH;

unsigned long            INPUTWND_START_POS_UP = 8;
...
MESSAGE_COLOR   messageColor[MESSAGE_TYPE_COUNT] = {
    ...
    {0, {0, 0, 150 << 8, 100 << 8}},
    {0, {0, 0, 0, 255 << 8}},
    {0, {0, 100 << 8, 100 << 8, 255 << 8}},
    {0, {0, 0, 0, 0}}
};

MESSAGE_COLOR   inputWindowLineColor = { 0, {0, 90 << 8, 160 << 8, 90 << 8} };  //输入框中的线条色
XColor          colorArrow = { 0, 255 << 8, 0, 0 }; //箭头的颜色

WINDOW_COLOR    inputWindowColor = { 0, 0, {0, 240 << 8, 255 << 8, 240 << 8} };
MESSAGE_COLOR   cursorColor = { 0, {0, 92 << 8, 210 << 8, 131 << 8} };

// *************************************************************
MESSAGE         messageUp[32];  //输入条上部分显示的内容
unsigned long            uMessageUp = 0;

// *************************************************************
MESSAGE         messageDown[32];    //输入条下部分显示的内容
unsigned long            uMessageDown = 0;
...
#else
//szj    iHeight = FontHeight (fontSet);
#endif
...
//szj       strcat (messageUp[0].strMsg, FCITX_VERSION);
...
#else
//szj   iInputWindowUpWidth += StringWidth (messageUp[i].strMsg, fontSet);
#endif
    }
...
#else
//szj   iInputWindowDownWidth += StringWidth (messageDown[i].strMsg, fontSet);
#endif
    }
...
/*  for (i = 0; i < uMessageUp; i++)
    {
//      printf("messageUp[i].strMsg is %s\n",messageUp[i].strMsg);
    ...
    for(j=0;j<2;j++)
    {
        for (i = 0; i < uMessageDown; i++)
        {
            printf("messageDown[i].strMsg is %s,i=%d\n",messageDown[j*uMessageDown+i].strMsg,j*uMessageDown+i);
        }
    }*/
...
//szj   OutputString (inputWindow, fontSet, strGBKT, iInputWindowUpWidth, (2 * iInputWindowHeight - 1) / 5, messageColor[messageUp[i].type].gc);
//  iPos = StringWidth (strGBKT, fontSet);
    ...
    if (bShowCursor && iChar) {
        if ((int)strlen (messageUp[i].strMsg) > iChar) {
...
#else
//szj       iCursorPixPos += StringWidth (strText, fontSet);
#endif
        iChar = 0;
        }
        else {
