...
bool UIPersonalCloudBookShelfPage::OnMessageListBoxUpdate(const EventArgs& args)
{
    if (PURCHARSEDSTATUS & m_wndStatus)
    {
        ...
        else
        {
            ...
            UpdateWindowStatus(PURCHARSEDSTATUS);
        }
    }
    return true;
}
...
bool UIPersonalCloudBookShelfPage::OnChildClick(UIWindow* child)
{
    ...
    else if (child == &m_gotoBookStore)
    {
        return OnGotoBookStoreClick();
    }
    return false;
}

bool UIPersonalCloudBookShelfPage::OnGotoBookStoreClick()
{
...
void UIPersonalCloudBookShelfPage::OnCommand(DWORD dwCmdId, UIWindow * pSender, DWORD dwParam)
{
    switch (dwCmdId)
    {
    case ID_BOOKSTORE_RETRY:
        {
            if (m_wndStatus & PURCHARSEDSTATUS)
            {
                OnPurchasedClick();
            }
            else if (m_wndStatus & UPLOADSTATUS)
            {
                OnUploadClick();
            }
            return;
    ...
    return UIPage::OnCommand(dwCmdId, pSender, dwParam);
}
...
string UIPersonalCloudBookShelfPage::GetSortString()
{
    ...
    {
        switch(m_cloudModelView.sortField())
        {
        case CREATE_TIME:
...
bool UIPersonalCloudBookShelfPage::OnSortClick()
{
    ...
    {
        if (m_cloudModelView.sortField() == sortTypes[i])
        {
            selected = i;
            break;
        }
    ...
    if (IDOK == menu.DoModal())
    {
        ...
        if (sort_field != m_cloudModelView.sortField())
        {
            int newSelected = -1;
            ...
            UpdateWindowStatus(UPLOADSTATUS);
...
void UIPersonalCloudBookShelfPage::Init()
{
    ...
    const int btnGotoFontSize = GetWindowFontSize(FontSize20Index);
    ...
    m_noPurchasedInfo.SetFontSize(GetWindowFontSize(FontSize24Index));
    m_gotoBookStore.Initialize(IDSTATIC,StringManager::GetStringById(CLOUD_GOTOBOOKSTORE),btnGotoFontSize);
...
//     m_freeBookList.SetMinSize(DeviceInfo::GetDisplayScreenWidth(), 3*GetWindowMetrics(UIPixelValue97Index));
    ...
    m_gotoUpload.Initialize(IDSTATIC,StringManager::GetStringById(CLOUD_UPLOADBOOK),btnGotoFontSize);
    ...
    m_noUploadInfo1.SetFontSize(GetWindowFontSize(FontSize24Index));
    m_noUploadInfo2.SetText(StringManager::GetPCSTRById(CLOUD_NOUPLOAD2));
    m_noUploadInfo2.SetFontSize(GetWindowFontSize(FontSize16Index));
    ...
    m_noUploadSpace.SetFontSize(GetWindowFontSize(FontSize20Index));
...
//     m_cloudModelView.SetLeftMargin(horizontalMargin);
    ...
    if (pMainSizer)
    {
        ...
        if (m_pPurcharsedNULLSizer)
        {
            ...
            m_pPurcharsedNULLSizer->Add(&m_imagePurcharsedNull, UISizerFlags().Align(dkALIGN_CENTER).Border(dkTOP, GetWindowMetrics(UIPixelValue40Index)).Border(dkBOTTOM, GetWindowMetrics(UIPixelValue26Index)));
            m_pPurcharsedNULLSizer->Add(&m_noPurchasedInfo, UISizerFlags().Align(dkALIGN_CENTER_HORIZONTAL));
            m_pPurcharsedNULLSizer->Add(&m_gotoBookStore, UISizerFlags().Expand().Align(dkALIGN_CENTER_HORIZONTAL).Border(dkTOP, lineSpacing));
            m_pPurcharsedNULLSizer->Add(&m_freeBookList, UISizerFlags(1).Expand().Border(dkBOTTOM, lineSpacing));
        ...
        if (m_pUploadNULLSizer)
        {
            ...
            m_pUploadNULLSizer->Add(&m_imageUploadNull, UISizerFlags().Align(dkALIGN_CENTER).Border(dkTOP, GetWindowMetrics(UIPixelValue50Index)).Border(dkBOTTOM, GetWindowMetrics(UIPixelValue40Index)));
            ...
            m_pUploadNULLSizer->Add(&m_noUploadSpace, UISizerFlags().Border(dkTOP | dkBOTTOM, GetWindowMetrics(UIPixelValue32Index)).Border(dkLEFT | dkRIGHT, horizontalMargin));
...
void UIPersonalCloudBookShelfPage::UpdateWindowStatus(WNDSTATUS wndStatus, bool needLayout)
{
    ...
    const bool noUpload = (wndStatus == NOUPLOAD);
    if (m_pUploadNULLSizer)
    {
        m_pUploadNULLSizer->Show(noUpload);
    }
    if (m_pUploadSizer)
    {
    ...
    m_fetchInfo.SetVisible((wndStatus == PURCHARSEDSTATUS) || (wndStatus == UPLOADSTATUS));
    m_editAll.SetVisible(wndStatus == UPLOAD);

    bool notOnLocal = (m_cloudModelView.sortField() == NOT_ON_LOCAL);
    if (noUpload)
    {
        m_noUploadInfo1.SetText(StringManager::GetPCSTRById(notOnLocal ? CLOUD_NOUPLOAD_NOT_ON_LOCAL : CLOUD_NOUPLOAD1));
    }
    ...
    m_btnTopGroup.SetVisible(wndStatus == UPLOAD || (noUpload && notOnLocal));
    if (needLayout)
    {
        Layout();
        UpdateWindow();
