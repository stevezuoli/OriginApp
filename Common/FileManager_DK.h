...
public:
    ...
    int GetBookCurSortNum() const
    {
        return m_sortedFiles.size();
    }
    DK_FileSorts    GetBookSort() const;
    void SetBookSortType(DK_FileSorts sort);
    CDKFile* GetBookbyListIndex(int index) const;
    ...
    int LoadFileToFileManger(const char* Path);
    int ReLoadFileToFileManger(const char* Path);
